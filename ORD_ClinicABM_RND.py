...
class ClinicAgent(ap.Agent):
    ...
    def choose_service(self):
        ...
        return [self.model.agent2clinic[c] for c in random.sample(self.children, k=self.model.p.choose_num_consult)] #[0]]
        #                                                      
...
class ClinicModel(ap.Model):
    ...
    def set_node_prob(self, node):
        ...
        return {
            'prob':random.uniform(0,1), #.np.sum([v for v in density.values()])/float(self.p.sum_consult),
            'choice_density':{
